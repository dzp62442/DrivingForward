...
class OmnisceneDataset(Dataset):
    ...
    def generate_depth_map(self, sample, sensor, cam_sample):
        ...
        # load and return if exists, generate if loading fails or file doesn't exist
        if os.path.exists(filename):
            try:
                return np.load(filename, allow_pickle=True)['depth']
            except Exception as e:
                print(f"Failed to load depth map from {filename}: {e}. Regenerating...")
        
        # Generate depth map if file doesn't exist or loading failed
